...
// $dataItem->price = 0;
        ...
        if ($chkDisplay) {
            ...
            if ($dataItem->type == 'picture') {
                ...
                $response .= '<div class="itemBloc ' . $colClass . ' lfb_picRow" data-itemtype="' . $dataItem->type . '">';
                $group = '';
                ...
                $response .= '<div class="itemBloc ' . $colClass . '" data-itemtype="' . $dataItem->type . '">';
                ...
                $response .= '<div class="itemBloc lfb_richtext  ' . $colClass . '" ' . $showInSummary . ' data-itemtype="' . $dataItem->type . '" data-itemid="' . $dataItem->id . '" data-title="' . $dataItem->title . '"  ' . $useShowConditions . ' ' . $showConditions . ' ' . $showConditionsOperator . '>' . do_shortcode($dataItem->richtext) . '</div>';
            } else if ($dataItem->type == 'shortcode') {
                $response .= '<div class="itemBloc lfb_richtext lfb_shortcode ' . $colClass . '" data-itemtype="' . $dataItem->type . '" data-itemid="' . $dataItem->id . '" data-title="' . $dataItem->title . '"  ' . $useShowConditions . ' ' . $showConditions . ' ' . $showConditionsOperator . '>' . do_shortcode($dataItem->shortcode) . '</div>';
            } else if ($dataItem->type == 'separator') {
                $response .= '<div data-itemid="' . $dataItem->id . '" data-itemtype="' . $dataItem->type . '" ' . $useShowConditions . ' ' . $showConditions . ' ' . $showConditionsOperator . '></div>';
                ...
                $response .= '<div class="itemBloc ' . $colClass . ' lfb_btnContainer" data-itemtype="' . $dataItem->type . '">';
                $group = '';
                if ($dataItem->groupitems != "") {
                    $group = 'data-group="' . $dataItem->groupitems . '"';
                }
        ...
        if ($form_id != "" && $form_id > 0 && !is_array($form_id)) {
            ...
            if (count($formReq) > 0) {
                ...
                foreach ($steps as $dataSlide) {
                    if ($dataSlide->formID == $form->id) {
                        ...
                        if (!empty($dataContent) && isset($dataContent->start)) {
                            ...
                            $shineCanvas = '';
                            $response .= '<a href="javascript:" id="lfb_btnNext_' . $dataSlide->id . '" class="btn btn-wide btn-primary btn-next ' . $hideNtxStepBtn . '">' . $nextStepIcon . $form->btn_step . '</a>';
               ...
               /* if ($form->enablePdfDownload) {
                    $response .= '<p><a href="javascript:" id="lfb_btnDownloadAsPDF" class="btn btn-default"><span class="fas fa-file-pdf"></span>' . $form->txtBtnDownloadPdf . '</a></p>';
                }*/
                ...
                if (!isset($gravity) || $gravity == 0) {
                    ...
                    if (isset($email) && $contactSent == 0) {
                        ...
                        if ($form->useGetResponse) {
                            ...
                            if ($customerInfos['phone'] == '') {
                                $customerInfos['phone'] = '_';
                            }
                            if ($customerInfos['country'] == '') {
                                $customerInfos['country'] = '_';
                            }
                            $GetResponse->addContact(array(
                                ...
                                'email' => $email,
                                'campaign' => array('campaignId' => $form->getResponseList),
                                'ipAddress' => $this->get_client_ip(),
                                'dayOfCycle' => 0,
                                'customFieldValues' => array(
                                    array(
                                        'customFieldId' => 'phone',
                                        'value' => array($customerInfos['phone'])
                                    ), array(
                                        'customFieldId' => 'city',
                                        'value' => array($customerInfos['city'])
                                    ), array(
                                        'customFieldId' => 'state',
                                        'value' => array($customerInfos['state'])
                                    ), array(
                                        'customFieldId' => 'postal_code',
                                        'value' => array($customerInfos['zip'])
                                    ), array(
                                        'customFieldId' => 'country',
                                        'value' => array($customerInfos['country'])
                                    )
                                )
                            ));
                        }
...
//variation
    ...
    private function encodeCalculation($calculation){
		...
		for ($i = 0; $i < strlen($calculation); $i++) {
			$encrypted .= array_key_exists($calculation[$i], $chars) 
				? $chars[$calculation[$i]]
				: $calculation[$i];
		}
