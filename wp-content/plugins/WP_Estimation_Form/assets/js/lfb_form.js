...
function wpe_initForms() {
    jQuery.each(wpe_forms, function () {
       ...
       /* jQuery('#estimation_popup.wpe_bootstraped[data-form="' + form.formID + '"] #lfb_btnDownloadAsPDF').on('click', function () {
            lfb_downloadAsPDF(form.formID);
        });*/
        var deviceAgent = navigator.userAgent.toLowerCase();
        var agentID = deviceAgent.match(/(iPad|iPhone|iPod)/i);
        if (agentID) {
    ...
    for (var o = 0; o < cloneSteps.length; o++) {
        ...
        if (($panel.attr('data-stepid') == 'final' || jQuery.inArray(parseInt($panel.attr('data-stepid')), lfb_plannedSteps) >= 0) && !jQuery('#estimation_popup.wpe_bootstraped[data-form="' + formID + '"] #mainPanel .genSlide[data-title][data-stepid="' + _stepID + '"]').is('.lfb_disabled')) {
            ...
            $panel.find('.itemBloc:not(.lfb_disabled)').each(function () {
                ...
                var itemData = lfb_getItemData(form, jQuery(this).find('[data-itemid]').attr('data-itemid'));
                items.push(itemData);
...
function wpe_cloneSummary(mode, formID) {
    /* var $originalSummary = jQuery('#estimation_popup.wpe_bootstraped[data-form="' + formID + '"] #lfb_summary');
     
     $originalSummary.find('td,th').each(function () {
     var width = (jQuery(this).width() * 100) / $originalSummary.find('table').width();
            ...
            if (item.label != undefined && item.label != "" && item.label != "undefined") {
                ...
                if (item.stepid != 'final' || form.summary_hideFinalStep != '1') {
                    if (item.stepid != step && !jQuery('#estimation_popup.wpe_bootstraped[data-form="' + formID + '"]').is('[data-sumhidesteps="1"]')) {
                        jQuery('#estimation_popup.wpe_bootstraped[data-form="' + formID + '"] #lfb_summary table tbody #lfb_summaryDiscountTr').before('<tr><th colspan="4" class="sfb_summaryStep" data-step="' + item.stepid + '">' + item.step + '</th>');
                    }
                    step = item.stepid;
                    ...
                    if (jQuery('#estimation_popup.wpe_bootstraped[data-form="' + formID + '"] [data-itemid="' + item.itemid + '"]').is('[data-hidezeropricesum="true"]') && item.price == 0) {
                        // if (!jQuery('#estimation_popup.wpe_bootstraped[data-form="' + formID + '"] [data-itemid="' + item.itemid + '"]').is('[data-type="numberfield"]') || jQuery('#estimation_popup.wpe_bootstraped[data-form="' + formID + '"] [data-itemid="' + item.itemid + '"]').is('[data-addtototal="1"]')) {
                        mustShow = false;
                        // }
...
function wpe_changeStep(stepID, formID) {
    ...
    //  var heightP = jQuery(' #estimation_popup.wpe_bootstraped[data-form="' + formID + '"] #mainPanel .genSlide[data-stepid="' + stepID + '"]').outerHeight()+parseInt(jQuery(' #estimation_popup.wpe_bootstraped[data-form="' + formID + '"] #mainPanel').css('padding-bottom')) + 102  /* + 18 + titleHeight */;
        ...
        if (jQuery('#estimation_popup.wpe_bootstraped[data-form="' + form.formID + '"]').is('[data-previousstepbtn="true"]') && stepID == 'final') {
            ...
            setTimeout(function () {
                ...
                if (lfb_lastSteps.length > 0) {
                    ...
                    // jQuery(' #estimation_popup.wpe_bootstraped[data-form="' + formID + '"] #mainPanel .genSlide[data-stepid="' + stepID + '"] .lfb_linkPreviousCt').delay(form.animationsSpeed * 3).fadeIn(500);
                }
            }, 1000);
...
function lfb_checkConditions(conditions, formID, _stepID) {
    ...
    jQuery.each(conditions, function () {
        ...
        if (condition.interaction.substr(0, 1) != '_' && condition.interaction.substr(0, 2) != 'v_') {
                ...
                if ($item.is('.lfb_disabled') || $item.closest('.genSlide').is('.lfb_disabled')) {
                    ...
                    if (condition.value && condition.value.indexOf('_') > -1) {
                        if (condition.value.substr(0, 1) != '_') {
                            ...
                            if(valueStepID == 'v'){
                                condition.value = lfb_getVariableByID(form, valueItemID).value;
            ...
            if (condition.interaction && condition.interaction.length > 6 && condition.interaction.substr(0, 6) == '_step-') {
...
function lfb_executeCalculation(calculation, formID, targetID) {
    ...
    if (jQuery('#estimation_popup.wpe_bootstraped[data-form="' + formID + '"]').is('[data-isSubs]') && itemData.isSinglePrice) {
                                            ...
                                            if (form.currencyPosition == 'left') {
                                                if (jQuery(this).is('[data-showprice="1"]')) {
                                                    if (jQuery(this).data('operation') == "+") {
                                                        ...
                                                        jQuery(this).attr('data-original-label', jQuery(this).data('originallabel') + ' : ' + form.currency + (wpe_formatPrice(valuesTab[reducIndex], formID)));
                                                ...
                                                if (jQuery(this).is('[data-showprice="1"]')) {
                                                    if (jQuery(this).attr('data-operation') == "+") {
                                                        ...
                                                        jQuery(this).attr('data-original-label', jQuery(this).data('originallabel') + ' : ' + (wpe_formatPrice(valuesTab[reducIndex], formID)) + form.currency);
    ...
    if (form.currencyPosition == 'left') {
        if ($item.is('[data-showprice="1"]')) {
            if ($item.data('operation') == "+") {
                ...
                $item.attr('data-original-label', $item.data('originallabel') + ' : ' + form.currency + (wpe_formatPrice($item.data('price'), formID)));
                ...
                $item.attr('data-original-label', $item.data('originallabel') + ' : +' + (wpe_formatPrice($item.data('price'), formID)) + '%');
...
function lfb_getDisabledHours(calendarID, day, eventDuration, eventDurationType) {
    ...
    if (calendar != false) {
        ...
        if(jQuery('#estimation_popup .lfb_datepicker[data-calendarid="' + calendar.id + '"]').attr('data-daysweek') == selDate.getDay()  || jQuery('#estimation_popup .lfb_datepicker[data-calendarid="' + calendar.id + '"]').attr('data-daysweek').split(',').indexOf(selDate.getDay()) > -1){
            for (var i = 0; i < 24; i++) {
