...
function tld_onOpen(){
     jQuery.ajax({
        ...
        success: function(url){            
            tld_previewUrl = url+'?rand='+Math.random();
...
function tld_tdgn_initMenu() {
    ...
    jQuery('#tld_tdgnContainer .tld_slider').each(function () {
     ...
     jQuery("#tld_tdgnContainer .lfb_bootstrap-select").click(function(){
...
function tld_getElementStyles(element) {
    ...
    jQuery('#tld_style_fontFamily').val($element.css('font-family').replace('"', '').replace('"', ''));
...
function tld_style_fontFamilyChange(){  
        if(tld_elementInitialized){
            ...
            if(!jQuery('#tld_style_fontFamily option:selected').is('[data-default="true"]')){
                var fontName = jQuery('#tld_style_fontFamily').val().replace(/ /g, '+');  
                tld_selectedElement.attr('data-googlefont','true');            
                jQuery('#tld_tdgnFrame').contents().find('head').append("<link href='https://fonts.googleapis.com/css?family=" + fontName+ "' rel='stylesheet' type='text/css'>");
            } else {                
                tld_selectedElement.removeAttr('data-googlefont');     
            }
        }
