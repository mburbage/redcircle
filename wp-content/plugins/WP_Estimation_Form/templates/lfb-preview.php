...
// scripts
wp_register_script($lfb->_token . '-frontend-libs', esc_url($lfb->assets_url) . 'js/lfb_frontendPackedLibs.min.js', array('jquery', "jquery-ui-core", "jquery-ui-tooltip", "jquery-ui-slider", "jquery-ui-position", "jquery-ui-datepicker"), $lfb->_version);
wp_enqueue_script($lfb->_token . '-frontend-libs');
wp_register_script('touchpunch', esc_url($lfb->assets_url) . 'js/jquery.ui.touch-punch.min.js', array($lfb->_token . '-frontend-libs'), $lfb->_version);
wp_enqueue_script('touchpunch');
wp_register_script($lfb->_token . '-estimationpopup', esc_url($lfb->assets_url) . 'js/lfb_form.min.js', array($lfb->_token . '-frontend-libs', 'touchpunch'), $lfb->_version);
...
if ($form) {
    ...
    if ($form->gmap_key != "") {
        ...
        if ($chkMap) {
            wp_register_script($lfb->_token . '-gmap', '//maps.googleapis.com/maps/api/js?key=' . $form->gmap_key . '&libraries=geometry' . $libPlace, array());
    ...
    $js_data[] = array(
        ...
        'imgPreview' => esc_url($lfb->assets_url) . 'img/file-3-128.png',
